...
namespace Kethane
{
    public struct Cell : IEquatable<Cell>
    {
        ...
        public int Level
        {
            get { return (LogBase2((Math.Max(2, index) - 2) / 5) + 1) / 2; }
        }
        ...
        private Cell approach(ChildType direction, int levels)
        {
            ...
            if (index < 2) { throw new ArgumentException("Cannot find child of a polar cell"); }
            var a = (uint)IntPow(3, (uint)levels);
        ...
        #region Bit hacks (TODO: Move to another class)
        ...
        private static int LogBase2(uint v)
        {
            v |= v >> 1;
            v |= v >> 2;
            v |= v >> 4;
